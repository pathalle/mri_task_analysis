...
correct_counts_per_stimulus$aStim = as.double(correct_counts_per_stimulus$aStim )
correct_counts_per_stimulus$block = "NA"
correct_counts_per_stimulus[which(correct_counts_per_stimulus$aStim<=4),]$block = "Block 1"
correct_counts_per_stimulus[which(correct_counts_per_stimulus$aStim>4),]$block = "Block 2"
correct_counts_per_stimulus$aStim = as.factor(correct_counts_per_stimulus$aStim )

correct_counts_per_stimulus <- cbind(correct_counts_per_stimulus,n_tot=counts_per_stimulus$n)

ggplot(correct_counts_per_stimulus, aes(x=aStim, y=accuracy, fill=block))+
  geom_bar(stat="identity", position=position_dodge()) +
  geom_hline(yintercept=0.75,linetype="dashed", color="red") +
  facet_wrap( ~ subj_idx, ncol=3) +
  scale_fill_manual(values = wes_palette("Darjeeling2", n = 4))+
  ggtitle("Accuracy per stimulus w/o missed, excluding 1,2 and 3rd trial of each pair")



last_four = rbind(data[which(data$aStim==1&data$trial_separate>=5&data$fb!=2),],
...
ggplot(l4_correct_counts_per_stimulus, aes(x=aStim, y=accuracy, fill=block))+
  ...
  facet_wrap( ~ subj_idx, ncol=3) +
  scale_fill_manual(values = wes_palette("Darjeeling2", n = 4))+
  ggtitle("Accuracy per stimulus of last 4 presentations per item (2x4) no missed trials")
