...
transformed parameters {
  ...
  eta_neg = 0.01*(exp(mu_pr[5] + sigma[5]) * fabs(eta_pr_neg));
  eta_pos = 0.01*(exp(mu_pr[6] + sigma[6]) * fabs(eta_pr_pos));
...
generated quantities {
  ...
  mu_eta_neg = exp(mu_pr[5])*0.01;
  mu_eta_pos = exp(mu_pr[6])*0.01;
  ...
  for (s in 1:N){
    ...
    for(trial in (first[s]):(last[s]-1)) {
      ...
      if (response[trial]==1){
        ...
        log_lik[trial] = wiener_lpdf(RT[trial] | alpha[s] * pow(iter[trial]/10,a_mod[s]),tau[s],0.5,-(delta_hat[trial]));
      }
      // if upper bound (resp = 2)
      else{
    ...
    if (response[last[s]]==1){
      log_lik[last[s]] = wiener_lpdf(RT[last[s]] | alpha[s] * pow(iter[last[s]]/10,a_mod[s]),tau[s],0.5,-(delta_hat[last[s]]));
    }
    if (response[last[s]]==2){
      log_lik[last[s]] = wiener_lpdf(RT[last[s]] | alpha[s] * pow(iter[last[s]]/10,a_mod[s]),tau[s],0.5,delta_hat[last[s]]);
      }
